			...
			try
			{
				...
				using (var conn = new NpgsqlConnection(connString))
				{
					...
					using (var cmd = new NpgsqlCommand())
					{
						cmd.Connection = conn;
						cmd.CommandText =
							$"REVOKE ALL PRIVILEGES ON archive.files_demand_archive_ers,archive.demand_archive_ers,archive.dem_arc_sts,archive.files_dem_arc_sts  FROM  {Current_user} ;";

						cmd.ExecuteNonQuery();
						cmd.CommandText =
							$"REVOKE ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA archive FROM {Current_user} ;";
						cmd.ExecuteNonQuery();
						...
						cmd.ExecuteNonQuery();
						cmd.CommandText =
							$"REVOKE ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA data  FROM {Current_user} ;";
						cmd.ExecuteNonQuery();
						cmd.CommandText =
							$"REVOKE ALL PRIVILEGES ON table data.cmr,data.files_cmr   FROM  {Current_user} ;";
						cmd.ExecuteNonQuery();
					}
					conn.Close();
					Current_user = "";
			...
			try
			{
				...
				using (var conn = new NpgsqlConnection(connString))
				{
					...
					using (var cmd = new NpgsqlCommand())
					{
						...
						cmd.ExecuteNonQuery();
						cmd.CommandText =
							$"GRANT SELECT, INSERT  ON   archive.files_demand_archive_ers,archive.demand_archive_ers,archive.dem_arc_sts,archive.files_dem_arc_sts  TO {Current_user} ;";
