...
namespace Geoportal.Services.Fabica_Shape
{
    public class Shape_draw : Shape
    {
        ...
        string geometry_multiPolgon;
        string geometry_multiLineString;
        string geometry_multiPoint;
        ...
        public Shape_draw(string name, string geometry_multiPolgon, string geometry_multiLineString, string geometry_multiPoint, string dbconn)
            ...
            try
            {
                ...
                using (var conn = new NpgsqlConnection(dbconn))
                {
                    ...
                    using (var cmd = new NpgsqlCommand())
                    {
                        ...
                        cmd.CommandText = "SELECT ST_AsText(ST_Union(ST_GeomFromText(@wkt_geometry_multiPolygon), ST_GeomFromText(@wkt_geometry_multiLineString) ) )";
                        ...
                        cmd.CommandText = "SELECT ST_AsText(ST_Union(ST_GeomFromText(@wkt_geometry_multiPoint), ST_GeomFromText(@wkt_geometry) ) )";
                        ...
                        cmd.CommandText = "INSERT INTO data.cmr(cmr_ident,cmr_name, geom,date_make) VALUES ('2',@name, ST_GeomFromText(@wkt_geometry,4326),'now') RETURNING cmr_id;";
